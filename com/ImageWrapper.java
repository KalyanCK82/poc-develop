...
public class ImageWrapper extends JFrame implements Runnable {
	...
	public ImageWrapper() {
		...
		try {
			...
			videoAdapter
					.loadResource("C:\\Users\\Public\\Videos\\Sample Videos\\snooker.wmv");

			imageOut = new MarvinImage(videoAdapter.getImageWidth(),
					videoAdapter.getImageHeight());
			imageBuffer = new MarvinImage(videoAdapter.getImageWidth(),
					videoAdapter.getImageHeight());
			...
			setSize((videoAdapter.getImageWidth() * 2) + 10,
					videoAdapter.getImageHeight() + 80);
			setVisible(true);
		...
		if (++fc == 8) {
			for (int y = 0; y < imgIn.getHeight(); y++) {
				for (int x = 0; x < imgIn.getWidth(); x++) {
					...
					// Is it not green?
					if (!(green > 120 && green > red * 1.4 && blue < 50)) {
